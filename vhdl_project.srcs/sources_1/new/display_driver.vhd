...
begin
    if (P_CLK'event and P_CLK='1') then
        if (RST='1') then
            ...
            if( ( horz_count >= ( h_pulse_width + h_back_porch ) ) and 
                ( horz_count <= ( h_pulse_width + h_back_porch + h_disp_period ) ) and 
                ( vert_count >= ( v_pulse_width + v_back_porch ) ) and 
                ( vert_count <= ( v_pulse_width + v_back_porch + v_disp_period ))) then
                
                DISP_EN <= '1';
                VALID   <= '1';
                X_POS   <= conv_std_logic_vector((horz_count-h_pulse_width-h_back_porch),9);
                Y_POS   <= conv_std_logic_vector((vert_count-v_pulse_width-v_back_porch),9);
            else
                DISP_EN <= '0';
                VALID   <= '0';
                X_POS   <= (others=>'0');
